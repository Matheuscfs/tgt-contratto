...
const ExplorePage: React.FC<ExplorePageProps> = ({ companies, categories, onSelectCompany, onToggleFavorite, initialCategory, onClearInitialCategory }) => {
    ...
    const filteredCompaniesBySearch = useMemo(() => {
        return companies.filter(company => {
            ...
            return searchTerm === '' || 
                   ...
                   company.services.some(s => s.name.toLowerCase().includes(searchTermLower)) ||
                   company.category.toLowerCase().includes(searchTermLower);
        });
    }, [companies, searchTerm]);

    const companiesForCategory = useMemo(() => {
        if (!selectedCategory) return [];
        return companies.filter(c => c.categoryId === selectedCategory.id);
        ...
        <div className="container mx-auto p-4 pb-20">
            ...
            <div className="relative mb-4">
                ...
                <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-full py-3 pl-11 pr-4 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary shadow-sm"
                />
            </div>
            ...
            {/* Categories Showcase */}
            {searchTerm === '' && (
                 <CategoryShowcase categories={categories} onSelectCategory={handleSelectCategory} />
            )}
